...
// Entries only. Two entries that share a helper make Rolldown emit a common chunk, and a
// chunk handed to `node --test` is a file with no tests in it - which passes, loudly.
const built = (await readdir(outDir)).filter((f) => f.endsWith('.gpu.mjs'));
let failed = 0;
