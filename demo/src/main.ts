...
async function main() {
  ...
  game.start((_dt) => {
    ...
    if (now - lastReport > 400) {
      ...
      status.textContent = `${canvas.width}x${canvas.height} · ${fps} fps · ${water.count} drops`;
      keysBox.innerHTML = row('page', `${page + 1}/${pages.length} — ${p.name}`)
        + row('goal', p.hint)
        + row('shape', `${shape}${ball.morphing ? ' …' : ''}`)
        + row('water', tapOn ? 'flowing (F)' : 'off (F)')
        + row('cleared', String(cleared))
        + row('keys', 'WASD roll · Space hop · 1/2/3 morph · Q dig · E add · F water · R reset · N next · drag to orbit');
      lastReport = now;
      lastFrames = frames;
    }
  });
